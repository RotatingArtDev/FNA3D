...
static void OPENGL_SetVertexBufferData(
	...
	OpenGLRenderer *renderer = (OpenGLRenderer*) driverData;
	OpenGLBuffer *glBuffer = (OpenGLBuffer*) buffer;
	uint64_t uploadBytes = (uint64_t) elementCount * (uint64_t) vertexStride;
	FNA3D_Command cmd;
		...
		renderer->supports_ARB_map_buffer_range	)
	{
		void *ptr = renderer->glMapBufferRange(
			...
			(GLsizeiptr) uploadBytes,
			GL_MAP_WRITE_BIT | GL_MAP_UNSYNCHRONIZED_BIT
		);
		if (ptr != NULL)
		{
			SDL_memcpy(ptr, data, uploadBytes);
			renderer->glUnmapBuffer(GL_ARRAY_BUFFER);
			if (renderer->perfDiagnosticsEnabled)
			{
				renderer->perfMapWrites += 1;
				renderer->perfMapBytes += uploadBytes;
				renderer->perfTotalMapWrites += 1;
				renderer->perfTotalMapBytes += uploadBytes;
				renderer->perfVertexUploadCalls += 1;
				renderer->perfVertexUploadBytes += uploadBytes;
				renderer->perfTotalVertexUploadCalls += 1;
				renderer->perfTotalVertexUploadBytes += uploadBytes;
			}
			return;
		}
	}
	...
	if (renderer->perfDiagnosticsEnabled)
	{
		...
		renderer->perfSubDataBytes += uploadBytes;
		renderer->perfTotalSubDataWrites += 1;
		renderer->perfTotalSubDataBytes += uploadBytes;
		renderer->perfVertexUploadCalls += 1;
		renderer->perfVertexUploadBytes += uploadBytes;
		renderer->perfTotalVertexUploadCalls += 1;
		renderer->perfTotalVertexUploadBytes += uploadBytes;
...
static void OPENGL_SetIndexBufferData(
	...
	OpenGLRenderer *renderer = (OpenGLRenderer*) driverData;
	OpenGLBuffer *glBuffer = (OpenGLBuffer*) buffer;
	uint64_t uploadBytes = (uint64_t) dataLength;
	FNA3D_Command cmd;
		...
		renderer->supports_ARB_map_buffer_range	)
	{
		...
		if (ptr != NULL)
		{
			...
			if (renderer->perfDiagnosticsEnabled)
			{
				...
				renderer->perfIndexUploadBytes += uploadBytes;
				renderer->perfTotalIndexUploadCalls += 1;
				renderer->perfTotalIndexUploadBytes += uploadBytes;
			}
			return;
		}
	}
